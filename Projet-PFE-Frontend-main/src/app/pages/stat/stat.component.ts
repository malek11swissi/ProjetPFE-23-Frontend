...
export class StatComponent implements OnInit {
  ...
  constructor(private typeTokensService: TypetokenService,private stat:StatistiqueAnnuelTokenService, private statistiqueannuelTokenService: StatistiqueAnnuelTokenService,
   ...
   for( let t of this.typetokens){
        ...
        for(let ele of this.res)
        {
          ...
          if(ele.typeToken==nom_token)
              {var index = this.categories.findIndex(element => element==ele.annee);
  ...
  reloadData() {
    ...
    this.typeTokensService.getTypeTokenList().subscribe(
      data => {
        this.typetokens = data;
        //console.log("Nombre des tokens"+ this.typetokens.length);
        
      });
    this.statistiqueAnnuelsToken = this.statistiqueannuelTokenService.getStatistiqueAnnuelsList();
  ...
  createChart() {
    ...
    this.statistiqueannuelTokenService.getChartStatsVenteToken().subscribe(
      res => {
